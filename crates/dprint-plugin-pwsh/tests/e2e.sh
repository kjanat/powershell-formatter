...
cargo build -p dprint-plugin-pwsh --profile wasm-release \
	--target wasm32-unknown-unknown
wasm="${repo_root}/target/wasm32-unknown-unknown/wasm-release/dprint_plugin_pwsh.wasm"
[[ -f "${wasm}" ]]
...
cat >dprint.json <<EOF
{
  "pwsh": { "indentWidth": 2, "frobnicate": true },
  "plugins": ["./plugin.wasm"]
}
EOF
