...
//! all formatting policy stays in `powershell-formatter`.

use powershell_formatter::{FormatOptions, FormatRange, JsonCatalog};
...
fn convert(result: powershell_formatter::FormatResult) -> JsFormatResult {
    JsFormatResult {
        text: result.text,
...
pub fn format(source: &str, options: JsValue, catalog: JsValue) -> Result<JsValue, JsValue> {
    ...
    let result = match &cat {
        Some(c) => powershell_formatter::format_with_catalog(source, &opts, c),
        None => powershell_formatter::format(source, &opts),
    };
...
pub fn format_range(
    ...
    let result = match &cat {
        Some(c) => powershell_formatter::format_range_with_catalog(source, &opts, c, range),
        None => powershell_formatter::format_range(source, &opts, range),
    };
    serde_wasm_bindgen::to_value(&convert(result)).map_err(|e| JsValue::from_str(&e.to_string()))
}
